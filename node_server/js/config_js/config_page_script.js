function myFunction( ){
    ...
    console.log( 'config file object - ' );
    ...
    function returnParentElement( propertyObj ){
        var attributes = propertyObj.parent_attributes, xpath = propertyObj.parent_xpath, tag = propertyObj.parent_tag, parentElement = null;
        try{
            /* if parent has `id` */
            if( attributes[ 'id' ] ) {
                parentElement = configPageObjDocument.getElementById( attributes[ 'id' ] );
                if( parentElement != null ){
                    return parentElement;
                }else{
                    //if an id is contains a dynamically generated entity
                    var temporary_id_container = attributes[ 'id' ],
                    split_id = temporary_id_container.split( /([0-9]+)/g );
                    for( var x = 0 ; x < split_id.length; x++ ){
                        var candidateParent = configPageObjDocument.querySelector( '*[id*="' + split_id[x] + '" ]' );
                        if( candidateParent )
            ...
            /* if parent has `class` */
            if( attributes[ 'class' ] ){
                var candidateParents = configPageObjDocument.getElementsByClassName( attributes[ 'class' ] );
                if( candidateParents.length > 0 ){
                    for( var i = 0; i < candidateParents.length; i++ ){
                        var candidateParent = candidateParents[i],
                        candidateParent_xpath = getXPathAutoScraper( candidateParent );
            ...
            if( condition_string != '' ){
                var candidateParent = configPageObjDocument.querySelector( tag + condition_string );
                ...
                candidateParent = configPageObjDocument.evaluate( xpath, configPageObjDocument, null, 9, null ).singleNodeValue;
                if( candidateParent != null )
                    parentElement = candidateParent;
            }else{
                var candidateParent = configPageObjDocument.evaluate( xpath, configPageObjDocument, null, 9, null ).singleNodeValue;
            ...
            console.log( ' returnParentElement() Code Have Error - ' + err.message );
            return parentElement;
        }
    }
    ...
    function mapConfig( ){
        for( var obj of dataObject ){
            if ( 'code_to_inject' in obj ){
                ...
                if( jsResult_ ){
                    console.log( 'Key - ' + obj.key + ' - Code to Inject result - ' + jsResult_ );
    ...
    function mapConfigSelectElementCheck( propertyObj ){
        ...
        var condition_string = '';
        var _targetElementAttrs = propertyObj.attributes;
        for( var attribute in _targetElementAttrs ){
            if( _targetElementAttrs[attribute] != '' )
                condition_string += '[' + attribute + '="' + _targetElementAttrs[attribute] + '" ]';
        }
        if( condition_string != '' ){
            // console.log( '\n\n1 - 1 - _targetElements - ', propertyObj.key);
            // console.log( '1 - 2 - condition_string - ', propertyObj.tag + condition_string);
            var _targetElements = configPageObjDocument.querySelectorAll( propertyObj.tag + condition_string + '' );
            var _targetParent   = returnParentElement( propertyObj );
            // console.log( '1 - 3 - _targetElements - ', _targetElements );
            // console.log( '1 - 4 - _targetParent - ', _targetParent );
            if( _targetElements.length >= 1 ){
                // console.log( '1 - 5 - _targetElements - ', propertyObj.key);
                for( var x = 0 ; x < _targetElements.length; x++ ){
                    if( _targetElements[x].parentElement === _targetParent  && _targetElements[x] != null ){
                        propertyValue_ = mapConfigSelectElementMapping( _targetElements[x], propertyObj );
                        // console.log( '1 - 6 - propertyValue_ - ',propertyValue_);
                        break;
                    }
                }
            }
        ...
        var targetElement = '';
        if ( propertyValue_ == '' && propertyObj.xpath ){
            ...
            // console.log( '2 - 1 - targetElement - ',targetElement);
        }
        if ( propertyValue_ == '' && targetElement ) {
            ...
            if ( propertyObj.tag.toLowerCase() == targetElement.tagName.toLowerCase() && propertyObj.parent_tag.toLowerCase() == targetElement.parentElement.tagName.toLowerCase() ) {
                ...
                if ( propertyObj.xpath == getXPathAutoScraper( targetElement ) && propertyObj.parent_xpath == getXPathAutoScraper( targetElement.parentElement ) ){
                    ...
                    if ( currentElementAttr.class && propertyObj.attributes.class ){
                        // console.log( '2 - 5 ' );
                        var _currentElementAttrClass = currentElementAttr.class.toString().replace( /\s+/g, ' ' ), _propertyObjAttributesClass = propertyObj.attributes.class.toString().replace( /\s+/g, ' ' );
                        ms_currentElementAttrClass = false;
                        ms_currentElementAttrClass = ( _currentElementAttrClass == _propertyObjAttributesClass ) ? true : false ;
                        // console.log( '2 - 5 - flag - '+ ms_currentElementAttrClass + ' - CEclass - '+_currentElementAttrClass + ' - POclass - '+_propertyObjAttributesClass );
                    }
                    // parent 
                    if ( currentElementParentAttr.id && propertyObj.parent_attributes.id ){
                        ...
                        // console.log( '2 - 6 - flag - '+ ms_currentElementParentAttrId + ' - CEid - '+currentElementParentAttr.id+ ' - POid - '+propertyObj.parent_attributes.id );
                    }
                    if ( currentElementParentAttr.class && propertyObj.parent_attributes.class ){
                        // console.log( '2 - 7 ' );
                        var _currentElementParentAttrClass = currentElementParentAttr.class.toString().replace( /\s+/g, ' ' ), _propertyObjParent_attributesClass = propertyObj.parent_attributes.class.toString().replace( /\s+/g, ' ' );
                        ms_currentElementParentAttrClass = false;
                        ms_currentElementParentAttrClass = ( _currentElementParentAttrClass == _propertyObjParent_attributesClass ) ? true : false ;
                        // console.log( '2 - 7 - flag - '+ ms_currentElementParentAttrClass + ' - CEPclass - '+currentElementParentAttr.class+ ' - POPclass - '+propertyObj.parent_attributes.class);
                    }
                }
            }
            if ( ms && ms_currentElementAttrClass && ms_currentElementParentAttrId && ms_currentElementParentAttrClass ){
                // console.log( '2 - 8 ' );
                propertyValue_ = mapConfigSelectElementMapping( targetElement, propertyObj );
                // console.log( '2 - 9 - value - '+propertyValue_);
            }
        }
    ...
    function getXPath( element ){
        ...
        console.log( 'Clicked Element Xpath - ' + x_paths );
    }
    ...
    function checkIsPropertyExist( targetElement, x, y ){
        ...
        if( classExistsFlag ){                            /* if already selected, unselect it and don't display label input box */
            ...
            if( _key ){
                ...
                showMessage( '', ' Property already selected, see high-lighted green border.', 'danger' );
            }
        }else{                                          /* if not selected, select it and display the label input box, so that user can enter a label */
            let image = ( window.getComputedStyle( targetElement ).backgroundImage ), labelItemValue = '';
    ...
    function postElement( label, targetElement ){
        let newDataObject = {}, child = targetElement, i = 0, jsResult = '';
        ...
        if ( jsResult == null || jsResult == undefined || jsResult == 'null' ) {
            $( '.alert' ).hide();        // hide all warning messages
            ...
            showMessage( label, ' - property successfully added, see record in table.', 'success' );
    ...
    /* function for label input `OK` button */
    function propertyBuilderWindow( labelItemValue ){
        $.confirm({
            ...
            buttons: {
                somethingElse: {
                    text: 'OK !', btnClass: 'btn-red ok-btn',
                ...
                cancel: { text: 'Cancel', btnClass: 'btn-danger display-none', keys: [ 'esc' ], action: function( ){} }
            }
        });
    }
    ...
    $(document).on( 'click', '#create_config', function( event ){
        $.confirm({
            title: 'Confirm!', icon: 'fa fa-warning', content: 'Save Config & Close the window',
            buttons: {
                confirm: {
                    text: 'Confirm', btnClass: 'btn-success mr-10', keys: [ 'enter' ],
                    action: function( ){
                        ...
                        if ( dataObject.length > 0 ) {
                            var data = { data: dataObject, url: host_, user_id: uid_, puppeteer_enabled: puppeteerEnabled }
                            ...
                            $.alert( { title: 'Error !', icon: 'fa fa-warning', content: 'Can\'t Create blank config', } );
                        }
                    }
                },
                cancel: { text: 'Cancel', btnClass: 'btn-danger', keys: ['esc'], action: function( ){} }
            }
        });
    });
    ...
    $( document ).on( 'click', '.del_prop_btn', function( ){
        var _key = this.getAttribute( 'key' ).toLowerCase(), _path = this.getAttribute( 'path' );
        ...
        showMessage( _key, ' - property is successfully removed.', 'info' );
    });

    // click for delete all property from property table
    $( document ).on( 'click', '#del_all_prop_btn', function( ){
        var dataObject = [], invalidPropertyObject = [], selected_items = configPageObjDocument.getElementsByClassName( 'option-selected' );
    ...
    $( document ).on( 'click', '#id_selector_btn', function( ){
        $.confirm({
            ...
            buttons: {
                somethingElse: {
                    text: 'OK !', btnClass: 'btn-red ok-btn', keys: ['enter'],
                    action: function( event ){
                        let label = document.getElementById( 'id_selector_ele_key' ).value, idSelectorText = document.getElementById( 'id_selector_ele_id' ).value, targetElement_ = configPageObjDocument.getElementById( idSelectorText );
                        console.log( " targetElement_ - "+targetElement_ );
    ...
    $( document ).on( 'click', '#invalid_property', function( ){
        var table = "", tableContent_ = '<tr><th><span id="del_all_invalid_prop_btn" title="Remove All Invalid properties">×</span></th><th>Key</th><th>Tag</th><th>Parent Tag</th></tr>';
        for( var invalidObject of invalidPropertyObject ){
